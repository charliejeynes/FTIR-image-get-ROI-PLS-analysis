...
for i = 1:2 %61 %53 %1:length(subfolders1)%r %[1] %[1 2 4 3 8 9 11 12 14 15 17 18 21]%[1 4 8 11 14 17 21]%1:length(filename)
%     to use the data on the datadrive need to add a bit in to read the
%     files
...
%% Sorts the spectra into WT or TG
groupIndx = repmat(group2wtTG, 1, length(amidenormMaster1)); %logical index to the same size as the Spectra
NumWT = numel(group2wtTG(group2wtTG == 1)); % gets how many WT 
WTb = amidenormMaster1(groupIndx); 
WT = reshape(WTb, NumWT, 457); % have to put in the correct number
TGb = amidenormMaster1(groupIndx == 0); 
NumTG = numel(group2wtTG(group2wtTG == 0));
TG = reshape(TGb, NumTG, 457); % have to put in the correct number
...
xlabel('normalise variables (wavenumbers)')
ylabel('normalised (with standard variance) absorbances ')

%% this puts it in standard format 
